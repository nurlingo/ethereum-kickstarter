...
describe("Campaigns", () => {
  ...
  it("requires a minimum contribution", async () => {
    try {
      ...
      assert(false); // the test is failed, as this point should not be reached
    } catch (err) {
      assert.ok(err.results); // assert(false) won't have results, but a real error will.
    }
  });
  ...
  it("processes requests", async () => {
    ...
    assert(postBalance - preBalance == 5);
  });
});
